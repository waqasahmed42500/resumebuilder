const Resume1 = ({ data, theme }) => {
  ...
  const contact = data?.contact ?? { fullName: "John Doe", headline: "Senior Product Designer", email: "john.doe@email.com", phone: "+1 212 555 0198", location: "New York, NY", website: "johndoe.design" };
  ...
  return (
    <article className="flex min-h-[1123px] w-[794px] overflow-hidden bg-white text-slate-700 shadow-2xl" style={{ fontFamily }}>
      <aside className="w-[248px] shrink-0 bg-slate-900 px-8 py-12 text-slate-200">
        ...
        <ResumeSection title="Education" dark>
          {education.map((entry) => <div key={entry.id} className="mb-4 last:mb-0"><p className="text-xs font-semibold text-white">{entry.degree}</p><p className="mt-1 text-xs text-slate-400">{entry.school}</p><p className="mt-1 text-[10px] text-slate-500">{entry.year}</p></div>)}
        </ResumeSection>
