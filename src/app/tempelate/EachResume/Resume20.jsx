...
const Resume20 = memo(function Resume20({ data: propData, theme, readOnly = false }) {
  ...
  return (
    <TemplateWrapper theme={theme} readOnly={readOnly} className="bg-white p-10 text-slate-900 min-h-[1123px]">
      {/* Zenith Centered Minimalist Header */}
      <header className="mb-6 text-center">
        ...
        <p className="mt-1 text-xs font-semibold uppercase tracking-[0.25em]" style={{ color: accent }}>
          <EditableText value={contact.headline} onChange={(val) => updateContact?.("headline", val)} readOnly={readOnly} placeholder="Headline" />
        </p>
        <div className="mt-2.5 flex justify-center gap-3 text-xs text-slate-500 flex-wrap">
        ...
        {portfolio.length > 0 && (
          <div className="mt-2 flex justify-center gap-3 text-xs text-slate-600 flex-wrap">
      ...
      <div className="border-t border-slate-200 pt-5 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6 border-t border-slate-200 pt-5">
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-1 mb-3 text-xs text-slate-700">
