...
const Resume7 = memo(function Resume7({ data: propData, theme, readOnly = false }) {
  const context = useResume();
  const data = propData || context?.resumeData;
  const accent = theme?.accent || context?.themeAccent || "#1e293b";
  ...
  return (
    <TemplateWrapper theme={theme} readOnly={readOnly} className="bg-white p-10 min-h-[1123px]">
      {/* Prestige Header */}
      <header className="mb-6 border-b-2 pb-5 flex justify-between items-start" style={{ borderColor: accent }}>
        <div>
        ...
        <div className="text-right text-xs leading-5 text-slate-600 space-y-0.5 font-sans">
      ...
      {/* Social Links Bar */}
      {portfolio.length > 0 && (
        <div className="mb-6 flex flex-wrap gap-4 text-xs text-slate-600 font-sans border-b border-slate-200 pb-3">
      ...
      <section className="mb-6">
        <h2 className="mb-2 text-xs font-bold uppercase tracking-[0.2em]" style={{ color: accent }}>
      ...
      <div className="grid grid-cols-2 gap-6 border-t border-slate-200 pt-5">
        <section>
          ...
          {education.map((entry, index) => (
            <div key={entry.id || index} className="group/item relative mb-2.5 last:mb-0">
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {skills.map((skill, index) => (
              <div key={index} className="group/item relative inline-flex items-center">
                <span className="rounded border border-slate-300 bg-slate-50 px-2 py-0.5 text-[10px] font-medium text-slate-800">
